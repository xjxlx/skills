...
2. 在候选文件中定位请求块。标准 `课程-方法名` 输入按课程和标题后缀匹配；若输入是含说明文字的完整标题，则按忽略空格、标点和全半角差异后的完整标题匹配。找不到或多个请求块匹配时停止并报告；定位后仍从请求体的 `service` 推导 Kotlin 方法名，不能只按 `service` 猜请求块。
3. 读取请求块中的方法、URL、请求头、表单字段和 `{{variable}}`；从请求文件所在目录向项目根目录查找最近的 `http-client.env.json`。只有一个环境时直接使用；多个环境优先使用唯一的 `dev`，仍有歧义时停止并报告，不能硬编码环境值。环境文件若含独占一行的 `//` 注释，解析时忽略这些注释行；其余 JSON 错误仍须报告。
