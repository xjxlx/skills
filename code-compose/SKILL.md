...
用户会在生成前把页面全部蓝湖图片下载到本地、按规范重命名并写入缓存（命名规范与缓存格式见 [references/lanhu-to-compose.md](references/lanhu-to-compose.md) 的“蓝湖资源命名与缓存”章节）：

- 资源命名与缓存：统一小写字母+数字命名（如 `组件 1.png` → `group1.png`），原始名称/URL 与本地文件名的映射必须记录到项目 `.codex/lanhu-resources.json`，生成时按缓存引用资源名
- 资源查找：生成时先查本地资源与缓存映射；找不到且蓝湖代码中存在对应下载资源时，立即停止生成，明确提示缺失资源清单，待用户补齐后继续
- 资源占位：找不到且蓝湖代码中也没有对应下载资源时，用同尺寸空白占位图临时占位，不阻塞生成，并在交付说明中列出待补资源清单
