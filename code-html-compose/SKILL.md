...
- 运行时尺寸适配优先复用目标 Activity 或项目已有的 AutoSize；已完成全局适配时直接使用项目的 `dp`/`sp`，不要再引入 `BoxWithConstraints`、局部 `Density` 或整页缩放。只有项目没有全局适配时，才按窗口宽高比例承载固定逻辑画布，并用设计背景色或背景图填充剩余空间，不能横纵分别拉伸或裁切。
