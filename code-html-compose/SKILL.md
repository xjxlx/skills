...
- 像素级验收宽高取当前设计包解析出的 `semantic.designW/designH`；不得固定为 1334×750 或其他历史尺寸。自动识别失败时要求成对设置 `DESIGN_WIDTH`、`DESIGN_HEIGHT`，禁止静默回退。
- 生成器按 `DP_PER_PX` 把当前设计尺寸和语义树坐标换算为固定逻辑画布；不得修改源坐标或用 `graphicsLayer` 整页缩放掩盖基线误差。
