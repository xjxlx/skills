...
- 坐标、尺寸、字号、行高和圆角固定按 `DP_PER_PX=0.5` 换算，保留半 dp/sp 精度；静态导入项目的 `AutoSizeConfig.DEFAULT_WIDTH` 和 `AutoSizeConfig.DEFAULT_HEIGHT`，布局中直接使用 `DEFAULT_WIDTH`、`DEFAULT_HEIGHT`，不要把 `375`、`667` 等具体 Android 基准值写入页面代码。
