...
- 本 Skill 按用户要求仅供本机 Codex 使用，不接入跨平台包装、公共发布或 `$skill-common` 的自动发布流程。
...
2. 目标 Android 项目根目录。
3. 目标 Compose 文件；如果文件尚不存在，确认页面名、模块和包路径。

复用用户已经提供的信息，不重复询问。无法确定目标文件或项目根目录时，不生成代码。

## 执行流程

### 1. 预检项目和设计包

- 确认 ZIP、Android 项目、目标模块和 Gradle Wrapper 可访问。
...
- 截图前通过当前 Activity、UI 结构或页面标识确认目标页面已经显示；只启动首页不算完成。
...
每次失败保留原始证据。只有稳定复现、能被编译/截图/布局数据验证，或用户明确要求固化的通用经验，才允许最小化更新本 Skill 或一层 reference。项目专属数值、单次偶发错误和未经验证的猜测不得写入规则；禁止追加按日期增长的事故日志。
