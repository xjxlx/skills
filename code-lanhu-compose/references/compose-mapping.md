...
进度条、分隔线、按钮、标签、纯色或简单圆角块等可由尺寸、颜色、形状和裁剪准确表达的视觉元素，优先使用 Compose 布局、`Shape` 或 `Canvas` 实现，不要用图片资源替代；按钮还必须保留 Compose 的点击和语义交互。只有纹理、复杂图案或特殊绘制效果无法可靠重建时才保留图片。进度填充层应继承轨道的可用高度（如 `fillMaxHeight()`），仅按比例调整宽度，并通过视觉对比验证结果。
